...
- `internal/billing` — `Service` talks to Stripe's REST API directly (no
  SDK): `Create` (customer), `Checkout`, `Portal`, plus webhook signature
...
**Out of scope (deliberate follow-up):** *plan enforcement* — billing records
the tier accurately, but no feature is gated on `plan` yet (that's a product
decision about which limits apply per tier). Metered/usage billing likewise.
